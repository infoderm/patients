...
const Chart = ({width, height, patients}) => {
	const today = startOfToday();
	const lt = (years) => (patient) =>
		isAfter(eidParseBirthdate(patient.birthdate), subYears(today, years));
	const gt = (years) => (patient) =>
		!isAfter(eidParseBirthdate(patient.birthdate), subYears(today, years));
	const data = [];
	let current = patients.filter((patient) => Boolean(patient.birthdate));
